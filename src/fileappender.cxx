...
{ 
	init(MINIMUM_ROLLING_LOG_SIZE, 1);
}
...
void
TimeBasedRollingFileAppender::init(long maxFileSize_, int maxBackupIndex_)
{
	if (maxFileSize_ < MINIMUM_ROLLING_LOG_SIZE)
	{
		tostringstream oss;
		oss << LOG4CPLUS_TEXT("TimeBasedRollingFileAppender: MaxFileSize property")
			LOG4CPLUS_TEXT(" value is too small. Resetting to ")
			<< MINIMUM_ROLLING_LOG_SIZE << ".";
		helpers::getLogLog().warn(oss.str());
		maxFileSize_ = MINIMUM_ROLLING_LOG_SIZE;
...
void 
TimeBasedRollingFileAppender::rollover1(bool alreadyLocked /*= false*/)
{
	helpers::LogLog & loglog = helpers::getLogLog();
	helpers::LockFileGuard guard;
	...
	if (useLockFile)
	{
		...
		helpers::FileInfo fi;
		if (getFileInfo(&fi, scheduledFilename) == -1
			|| fi.size < maxFileSize)
		{
			// The file has already been rolled by another
			// process. Just reopen with the new file.
